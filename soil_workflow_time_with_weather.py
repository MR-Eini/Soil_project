...
Optional additional predictors:
  - daily weather predictors (joined by Dayn as day-of-year for 2019), plus lag features:
      Tmp_max, Tmp_min, HMD, WND, PCP and <var>_lag1..lag7
    Weather file is expected to cover 2019-01-01 .. 2019-12-31 (365 rows).
...
DEFAULT_WEATHER_XLSX = BASE_DIR / "data_weather.xlsx"  # 2019 daily weather
...
def read_weather_xlsx(weather_path: Path, date_col: str = "Date", max_lag_days: int = 7) -> pd.DataFrame:
    """
    Reads daily weather (expected: 2019 daily series) and adds a Dayn join key as day-of-year (1..365/366).

    Also creates lag features for each numeric weather column:
      <col>_lag1 ... <col>_lag{max_lag_days}

    Expected columns (typical):
      Date, Tmp_max, Tmp_min, HMD, WND, PCP
    """
    w = pd.read_excel(weather_path)
    w.columns = [c.strip() for c in w.columns]
    if date_col not in w.columns:
        raise ValueError(f"Weather file must contain date column '{date_col}'. Found: {w.columns.tolist()}")

    w[date_col] = pd.to_datetime(w[date_col], errors="coerce")
    if w[date_col].isna().any():
        bad = int(w[date_col].isna().sum())
        raise ValueError(f"Weather file has {bad} rows with non-parsable dates in '{date_col}'.")

    # sort by date (important for lag generation)
    w = w.sort_values(date_col).reset_index(drop=True)

    # defensive check for missing days (affects lag semantics)
    if len(w) > 2:
        diffs = w[date_col].diff().dt.days.dropna()
        gap_count = int((diffs != 1).sum())
        if gap_count > 0:
            warnings.warn(
                f"Weather dates are not a continuous daily series; found {gap_count} non-1-day gaps. "
                f"Lag features will follow row order, not calendar gaps.",
                RuntimeWarning,
            )

    w["Dayn"] = w[date_col].dt.dayofyear.astype(int)

    # keep only one row per Dayn (defensive)
    w = w.drop_duplicates(subset=["Dayn"], keep="first").reset_index(drop=True)

    # convert non-date columns to numeric
    for c in w.columns:
        if c in ("Dayn", date_col):
            continue
        w[c] = pd.to_numeric(w[c], errors="coerce")

    # create lag features for base variables (previous days)
    base_cols = [c for c in w.columns if c not in ("Dayn", date_col) and not re.search(r"_lag\d+$", c)]
    for c in base_cols:
        for k in range(1, int(max_lag_days) + 1):
            w[f"{c}_lag{k}"] = w[c].shift(k)
...
def main():
    ...
    # --- Weather options ---
    parser.add_argument("--add_weather", action="store_true",
                        help="If set, joins daily weather predictors from an Excel file (2019) by Dayn (day-of-year), and adds lag1..lagN features (see --weather_lag_days).")
    parser.add_argument("--weather_xlsx", type=str, default=str(DEFAULT_WEATHER_XLSX),
                        help="Path to weather Excel file (must include a date column; default expects 'Date').")
    parser.add_argument("--weather_date_col", type=str, default="Date",
                        help="Date column name in the weather Excel file (default: Date).")
    parser.add_argument("--weather_features", type=str, default="",
                        help="Comma-separated list of base weather columns to include (e.g., PCP,Tmp_max). For each selected base variable, lag1..lagN is also added (see --weather_lag_days). Default: all numeric columns except Date, including generated lag columns.")


    parser.add_argument("--weather_lag_days", type=int, default=7,
                        help="Number of daily lag features to add for each selected weather variable (default: 7).")
    ...
    weather_path = Path(args.weather_xlsx)
    ...
    # --- Subset to rows with target ---
    y = pd.to_numeric(df[args.target], errors="coerce").to_numpy()
    ok = ~np.isnan(y)
    df2 = df.loc[ok].reset_index(drop=True)
    y = y[ok]

    # --- Optional weather join (adds columns to df2) ---
    weather_cols: list[str] = []
    if args.add_weather:
        if not weather_path.exists():
            raise FileNotFoundError(f"Weather xlsx not found: {weather_path}")
        log(f"Reading weather xlsx: {weather_path}", log_file)
        t_w0 = time.perf_counter()
        w = read_weather_xlsx(weather_path, date_col=args.weather_date_col, max_lag_days=args.weather_lag_days)

                # choose weather feature columns (include lags up to --weather_lag_days)
        requested = parse_list_arg(args.weather_features)
        if requested:
            missing = [c for c in requested if c not in w.columns]
            if missing:
                raise ValueError(
                    f"Requested weather features not in weather file: {missing}. Available: {w.columns.tolist()}"
                )

            expanded: list[str] = []
            for c in requested:
                expanded.append(c)
                # auto-expand base (non-lag) variables to include lag1..lagN
                if not re.search(r"_lag\d+$", c):
                    for k in range(1, int(args.weather_lag_days) + 1):
                        lc = f"{c}_lag{k}"
                        if lc in w.columns:
                            expanded.append(lc)

            # preserve order, drop duplicates
            weather_cols = list(dict.fromkeys(expanded))
        else:
            weather_cols = [c for c in w.columns if c not in ("Dayn", args.weather_date_col)]
            # keep only columns that are not completely NA
            weather_cols = [c for c in weather_cols if not w[c].isna().all()]

        # ensure join key types
        df2["Dayn"] = pd.to_numeric(df2["Dayn"], errors="coerce").astype(int)
        w["Dayn"] = pd.to_numeric(w["Dayn"], errors="coerce").astype(int)

        # join
        df2 = df2.merge(w[["Dayn"] + weather_cols], on="Dayn", how="left", validate="many_to_one")

        # log join quality
        miss_any = df2[weather_cols].isna().any(axis=1).sum()
        miss_all = df2[weather_cols].isna().all(axis=1).sum()
        log(
            f"Weather joined | cols={weather_cols} | rows_with_any_missing={int(miss_any)} | rows_with_all_missing={int(miss_all)} | "
            f"elapsed={fmt_seconds(time.perf_counter() - t_w0)}",
            log_file,
        )

    # --- Determine remote predictors (optionally metadata-filtered) ---
    kept_remote = remote_cols
    if args.domain_filter:
        ...
        kept_remote = filter_by_index_metadata(remote_cols, xlsx_path, domain=args.domain_filter)
        log(f"Domain filter kept remote predictors | count={len(kept_remote)}", log_file)

    # --- Build X ---
    log("Preparing predictors...", log_file)
    t0 = time.perf_counter()

    predictor_cols = list(kept_remote)
    if args.add_weather and weather_cols:
        predictor_cols += weather_cols

    X = df2[predictor_cols].copy()
